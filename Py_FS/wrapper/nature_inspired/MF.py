...
def update_max_velocity(male, female):
    ...
    for j in range(length):
        ...
        agent1.append((male[0][j]-male[size-1][j])*r) #Eq 16
...
def update_velocity(m_pos, f_pos, m_vel, f_vel, Leader_agent, pbest, a1, a2, d, fl, g, b, agent_number, data, obj_function):
    ...
    if agent_number==0:
        for j in range(tot_features):
            agent1[j] = m_vel[j]+d*np.random.uniform(-1,1)  #Eq 11
    else:
        sum = 0
        for j in range(tot_features):
            sum = sum+(m_pos[j]-Leader_agent[j])*(m_pos[j]-Leader_agent[j])
        rg = math.sqrt(sum) #Eq 10
        sum = 0
        for j in range(tot_features):
            sum = sum+(m_pos[j]-pbest[j])*(m_pos[j]-pbest[j])
        rp = math.sqrt(sum)
        for j in range(tot_features):
            agent1[j] = g*m_vel[j]+a1*math.exp(-b*rp*rp)*(pbest[j]-m_pos[j])+a2*math.exp(-b*rg*rg)*(Leader_agent[j]-m_pos[j]) #Eq 17
    ...
    if obj_function(m_pos, data.train_X, data.val_X, data.train_Y, data.val_Y) >= obj_function(f_pos, data.train_X, data.val_X, data.train_Y, data.val_Y):
        ...
        rmf = math.sqrt(sum)
        agent2[j] = g*f_vel[j]+a2*math.exp(-b*rmf*rmf)*(m_pos[j]-f_pos[j])  #Eq 18.1
    else:
        for j in range(tot_features):
            agent2[j] = g*f_vel[j]+fl*np.random.uniform(-1,1) #Eq 18.2
...
def check_velocity_limits(m_vel, f_vel, vmax_m, vmax_f):
    ...
    for j in range(tot_features):
        m_vel[j] = np.minimum(m_vel[j], vmax_m[j])  #Eq 15.1
        m_vel[j] = np.maximum(m_vel[j], -vmax_m[j]) #Eq 15.2
        f_vel[j] = np.minimum(f_vel[j], vmax_f[j])  #Eq 15.1
...
def compare_and_replace(pos, off, fit, data, obj_function):
    ...
    temp_pos, newfit = sort_agents(off, obj_function, data)
    i=0
    j=0
    cnt=0
    ...
    while(cnt < agents):
        if fit[i] > newfit[j]:
            temp_pos[i] = pos[i].copy()
            i+=1
        else:
            temp_pos[i] = off[i].copy()
            j+=1
        cnt+=1
    return temp_pos
