...
def Mayfly(num_agents, max_iter, train_data, train_label, obj_function=compute_accuracy, trans_function_shape='s', mut_prob=0.2, delta=0.8):
    # Mayfly Algorithm
    ############################### Parameters ####################################
    #                                                                             #
...
def update_velocity(m_pos, f_pos, m_vel, f_vel, Leader_agent, pbest, a1, a2, d, fl, g, b, i, data, obj_function):
    ...
    if i==0:
        ...
        rp = math.sqrt(sum)
        for j in range(tot_features):
            agent1[j] = g*m_vel[j]+a1*math.exp(-b*rp*rp)*(pbest[j]-m_pos[j])+a2*math.exp(-b*rg*rg)*(Leader_agent[j]-m_pos[j])
    if obj_function(m_pos, data.train_X, data.val_X, data.train_Y, data.val_Y) >= obj_function(f_pos, data.train_X, data.val_X, data.train_Y, data.val_Y):
        ...
        rmf = math.sqrt(sum)
        agent2[j] = g*f_vel[j]+a2*math.exp(-b*rmf*rmf)*(m_pos[j]-f_pos[j])
    else:
        for j in range(tot_features):
